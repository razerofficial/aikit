...
void runGemm(at::Tensor& D, at::Tensor const& A, at::Tensor const& B,
             ...
             cudaStream_t stream, double& gflops, int warmup_iteration, int iteration, bool verbose) {
  Gemm gemm;
  ...
  gflops = 0.0f;
  if (iteration > 0) {
      // Make sure GPU is idle before starting timer
      ...
      // Wait for all kernels to finish before stopping timer
      cudaDeviceSynchronize();
      ...
      double avg_runtime_ms = double(elapsed_ms) / double(iteration);
      gflops = GFLOPS(M, N, K, avg_runtime_ms / 1000.0);
...
void cutlass_fp4_bf16_gemm_dispatch(torch::Tensor& D, torch::Tensor const& A,
  ...
  if (mp2 <= 256) {
    runGemm<Fp4GemmSm120<sm120_fp4_config_M256, cutlass::bfloat16_t>::Gemm>(
        D, A, B, A_sf, B_sf, alpha, m, n, k, stream, gflops, warmup_iteration, iteration, verbose);
  } else {
...
void cutlass_scaled_fp4_mm_sm120a(torch::Tensor& D, torch::Tensor const& A,
  ...
  if (out_dtype == at::ScalarType::BFloat16) {
    return cutlass_fp4_bf16_gemm_dispatch(D, A, B, A_sf, B_sf, alpha, m, n, k,
                                          stream, gflops, warmup_iteration, iteration, verbose);
